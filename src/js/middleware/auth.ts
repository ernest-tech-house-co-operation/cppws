import type { SecurityConfig, AuthConfig, WSContext } from '../../types/index.js';
 ...
 */
function verifySimpleJWT(token: string, secret: string): Record<string, any> {
    ...
    const expectedSig = base64UrlEncode(
        createHmacSha256(secret, `${headerB64}.${payloadB64}`)
    );
 ...
 */
function createHmacSha256(key: string, data: string): Buffer {
 ...
 */
export function createAuthMiddleware(security: SecurityConfig) {
    const auth = security.auth;
    if (!auth || !auth.enabled) {
        // Auth not enabled — pass through
        return async (_headers: Record<string, string>, _query: Record<string, string>) => {
            return null;
        };
    }
